...
sql1 = "DELETE FROM census_info_beachhouse"

cursor.execute(sql1)

sql2 = '''COPY census_info_beachhouse(instance_id, patient_record_key, patient_master_key,
 first_name, last_name, mr, admission_date, program_name, length_of_stay, current_ur_loc,
  current_clinical_loc, insurance_company, age, sex, ethnicity, diagcode_list,
   diagcodename_list, primarycareteam_primarytherapist, statuses, iscensus,
    paymentmethod)
FROM 'C:/SQL_CSV/census_info_beachhouse.csv'
DELIMITER ','
CSV HEADER;'''

cursor.execute(sql2)

"""sql3 = '''select * from census_info_beachhouse;'''
cursor.execute(sql3)
